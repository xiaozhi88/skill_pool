...
def SVM_training(train_x, train_y, C, toler, max_iter, kernel_option = ('lin', 0)):
    '''SVM的训练
    input:  train_x(mat):训练数据的特征
            train_y(mat):训练数据的标签save_svm_model
            C(float):惩罚系数
...
class SvmVo(object):
    def __init__(self, trainDataPath, testDataPath, modelPath):
        self.trainDataPath = trainDataPath
        self.testDataPath = testDataPath
        self.modelPath = modelPath

def dictToSvmVo(d):
    return SvmVo(d["trainDataPath"], d["testDataPath"], d["modelPath"])
